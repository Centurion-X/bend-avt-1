...
class Arena
{
    ...
    check ()
    {
        if (this.amount != 0 && this.decepticon.health > 0)
        {
            this.fight();
            this.render();
            if (this.autobots[this.index].health <= 0) this.autobots.splice(this.index, 1);
        }
        else
        {
    ...
    comment ( text = '' )
    {
        ...
        else string = document.createElement('br');
    ...
    fight ()
    {
        ...
        ( 
            (bot, index, array) =>
            {
                if (bot && bot.health > 0) this.decepticon.hit(bot.attack())
                else this.amount--;
            }
        );
        if (this.decepticon.health <= 0) side_two.parentElement.setAttribute('hidden', true);
        this.round++;
    }
    ...
    render ()
    {
        ...
        this.autobots.forEach((bot, index) =>
        {
            if (bot.health > 0)
            {
                ...
                {
                    if (!condition) condition = 'norms';
                    hit.call(this, 'bot');
                    this.comment(`Робот ${bot.info} стреляет из оружия и наносит урон в ${bot.weapon.damage} hp.`);
                }
                else this.comment(`Робот ${bot.info} промахивается.`);
            }
        });
        if (!condition) condition = 'dodge';
        ...
        function hit ( bot_id, bot = this.decepticon )
        {
            ...
            if (bot_id.length > 3) bot = this.autobots[this.index];
    ...
    speak ( condition )
    {
        ...
        if (phrase && bubble.textContent !== phrase) bubble.textContent = phrase
        else this.speak(condition);
    }
    ...
    start ()
    {
        ...
        if (side_two.parentElement.hasAttribute('hidden'))
        {
            side_two.classList.remove('hit');
            side_two.parentElement.removeAttribute('hidden');
        }
